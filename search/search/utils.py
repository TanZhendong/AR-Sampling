...
def generate_multi_steps(
    ...
    for i, text in enumerate(templated_convs):
        gen_result = GenResult(
            index=i,
            initial_prompt=text,
            first_step_text="", # no use for this case 
            first_step_stop_reason=None, # no use
            lookahead_text="", 
            stop_reason=None,
        )
        gen_results.append(gen_result)
    ...
    for cur_step in range(max_steps):
        ...
        llm_outputs = llm.generate(gen_prompts, sampling_params, use_tqdm=False)
